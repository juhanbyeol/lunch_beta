	...
	requestPostNoBlockUI("selectBoard", "json", null, function(res) {
		...
		if(msg == 'Y') {
			...
			for (var i = 0; i < result.length; i++) {
				//구내식당 상태 
				console.log(result[i].foodState);
							...
							<div class="lunchFood">
								...
								<div class="lunchRight">
									...
									<div class="lunchDataBox lunchDataBox2">
										...
										${foodStateTag}
										<button type="button" class="btn btn1" id="foodStateBtn" data-sn="${result[i].boardSn}">변경하기</button>
									</div>
	...
	requestPostNoBlockUI("selectBoardComment", "json", null, function(res) {
		...
		if(msg == 'Y') {
			...
			for (var i = 0; i < result.length; i++) {
				...
				li += `<li>
					   		<div>	
								<p class="comt comt1">${result[i].ncnm}</p>		
								<p class="comt comt2">${result[i].cn}</p>		
								<p class="comt comt3">${result[i].registDt}</p>							
							</div>
							<div class="comt comt4">
								<div class="btn btn3">
									<button type="button" style="color:${likeBtnColor};" id="likeBtn" data-sn="${result[i].commentSn}">♥</button>
